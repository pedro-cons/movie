...
describe('MoviesService', () => {
  ...
  beforeEach(async () => {
    ...
    actorsRepository = module.get<MockRepository<Actor>>(getRepositoryToken(Actor));
  });
  ...
  describe('findOne', () => {
    ...
    it('throws NotFoundException with the correct id in message', async () => {
      ...
      await expect(service.findOne(42)).rejects.toThrow('Movie with ID 42 not found');
    });
  });
  ...
  describe('create', () => {
    it('creates a movie without actors', async () => {
      ...
      expect(moviesRepository.create).toHaveBeenCalledWith({ title: 'New Movie', genre: 'Drama' });
  ...
  describe('update', () => {
    it('updates the movie fields', async () => {
      const existingMovie = { id: 1, title: 'Old Title', genre: 'Action', actors: [] };
    ...
    it('replaces actors when actorIds is provided', async () => {
      ...
      moviesRepository.save.mockResolvedValue({ ...existingMovie, actors: newActors });
    ...
    it('does not modify actors when actorIds is undefined', async () => {
      ...
      moviesRepository.findOne.mockResolvedValue(existingMovie);
      moviesRepository.save.mockResolvedValue({ ...existingMovie, title: 'Updated Title' });
