...
describe('UsersService', () => {
  ...
  describe('findByUsername', () => {
    it('returns a user when found by username', async () => {
      ...
      expect(usersRepository.findOne).toHaveBeenCalledWith({ where: { username: 'alice' } });
    ...
    it('returns null when no user is found for the given username', async () => {
      ...
      expect(usersRepository.findOne).toHaveBeenCalledWith({ where: { username: 'unknown' } });
  ...
  describe('create', () => {
    ...
    it('returns the saved user entity including the generated id', async () => {
      ...
      usersRepository.create.mockReturnValue({ username: 'carol', password: 'carol_hash' });
